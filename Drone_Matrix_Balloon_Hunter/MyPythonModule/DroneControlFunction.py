...
def execute_function_in_queue(queue):
    ...
    while True:
        ...
        if func == 'End':
            print('{} - execute_function_in_queue() is terminated.\n'.format(time.ctime()))
            return
        else:
...
def CLIENT_send_immediate_command(remote_host, immediate_command_str):
    ...
    try:
        client_socket.connect((remote_host, __builtin__.port_immediate_command))
...
def send_body_frame_velocity_once(vehicle, velocity_x, velocity_y, velocity_z):
    ...
    """
    print('\n')
    print('{} - Calling function send_body_frame_velocity(Vx={}, Vy={}, Vz={})'.format(time.ctime(), velocity_x, velocity_y, velocity_z))
    msg = vehicle.message_factory.set_position_target_local_ned_encode(
        0,       # time_boot_ms (not used)
        0, 0,    # target system, target component
        mavutil.mavlink.MAV_FRAME_BODY_OFFSET_NED, # frame
...
def curvature_flight_body_frame(vehicle, horizontal_linear_speed, radius_of_curvature, total_turn_degree_deg, velocity_z, atom_segment):
    print('\n')
    print('{} - Calling function curvature_flight_body_frame(horizontal_linear_speed={}, radius_of_curvature={}, total_turn_degree_deg={}, velocity_z={})'.format(time.ctime(), horizontal_linear_speed, radius_of_curvature, total_turn_degree_deg, velocity_z))
...
def takeoff(vehicle, wait_time):
    ...
    vehicle.simple_takeoff(1) # Take off 1 meter. Just take off so we can send other movement command.
